{ config, pkgs, lib, ... }: {
    ...
    programs.zsh.shellAliases = lib.mkIf config.programs.zsh.enable {
	    ...
	    playlist = "${pkgs.fd}/bin/fd . ${config.home.homeDirectory}/sync/archive/wallpaper/video/playlists/ -e m3u | ${pkgs.skim}/bin/sk | xargs ${pkgs.mpvpaper}/bin/mpvpaper -v -p -o 'loop-file=inf --no-resume-playback --panscan=1' '*'";
